...
namespace iTextSharp.text.pdf.parser {
	...
	public class TextRenderInfo {
		...
		private readonly GraphicsState gs;
		private readonly double[] fontMatrix = null;
		 ...
		 * @since 5.0.2
		 */
		private readonly ICollection<MarkedContentInfo> markedContentInfos;
		...
		private LineSegment GetUnscaledBaselineWithOffset(float yOffset) {
			...
			var correctedUnscaledWidth = GetUnscaledWidth() - (gs.characterSpacing +
					(unicodeStr.Length > 0 && unicodeStr[^1] == ' ' ? gs.wordSpacing : 0)) * gs.horizontalScaling;
		...
		public virtual float GetRise() {
			if (gs.rise == 0) {
				return 0; // optimize the common case
			}
		...
		private Encoding Utf_16BeEncoding {
			get {
				return utf_16BeEncoding ?? (utf_16BeEncoding = Encoding.GetEncoding("UTF-16BE"));
			}
		}
	}
}
